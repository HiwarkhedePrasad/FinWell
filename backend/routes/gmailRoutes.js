    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        try {
          ...
          const senderEmail =
            emailData.headers.from?.match(/<(.+)>/)?.[1] ||
            emailData.headers.from;

          console.log(`Sender email: ${senderEmail}`);

          // Find user by email - for testing, let's use a fallback
          let user = null;
          if (senderEmail) {
            user = await User.findOne({ email: senderEmail });
          }

          // If no user found by sender email, try to get any user for testing
          if (!user) {
            console.warn(
              `No user found for email: ${senderEmail}, using first available user`
            );
            user = await User.findOne(); // Get any user for testing
          }
